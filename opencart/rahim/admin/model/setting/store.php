...
class Store extends \Opencart\System\Engine\Model {
	...
	public function getStores(): array {
		...
		$merchant_check = "SELECT ug.`name` 
						   FROM `" . DB_PREFIX . "user` u 
						   INNER JOIN `" . DB_PREFIX . "user_group` ug 
		...
		if ($role_name === 'Merchants') {
			// Query for merchants to fetch stores assigned to the merchant
			$sql = "SELECT s.* 
					FROM `" . DB_PREFIX . "store` s 
					INNER JOIN `" . DB_PREFIX . "merchant_store_relation` msr 
					ON s.`store_id` = msr.`store_id` 
					WHERE msr.`user_id` = " . $user_id . " 
					ORDER BY s.`url`";
		} else {
			// Query for admin to fetch all stores
			$sql = "SELECT * FROM `" . DB_PREFIX . "store` ORDER BY `url`";
		}
	
		// Cache key based on the query
		$cache_key = 'store.' . md5($sql);
	
		// Check if the store data is in the cache
		$store_data = $this->cache->get($cache_key);
	
		if (!$store_data) {
			// Execute the query if the data is not cached
			...
			// Cache the result
			$this->cache->set($cache_key, $store_data);
		}
