...
class Category extends \Opencart\System\Engine\Model {
	...
	public function getCategories(array $data = []): array {
			...
			if($role_name == "Merchants"){
				...
				AS `name`, c1.`parent_id`, c1.`sort_order`, c1.`status` 
				...
				LEFT JOIN `" . DB_PREFIX . "category_to_store` cts 
				ON (cp.`category_id` = cts.`category_id`) 
				WHERE cd1.`language_id` = '" . (int)$this->config->get('config_language_id') . "' 
				AND cd2.`language_id` = '" . (int)$this->config->get('config_language_id') . "'
				AND cts.`store_id` IN (SELECT store_id 
